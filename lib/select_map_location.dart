...
class CustomOpenStreetMapSearchAndPick extends StatefulWidget {
  final CustomLatLong center;
  final void Function(CustomPickedData pickedData) onPicked;
  ...
  const CustomOpenStreetMapSearchAndPick(
      {Key? key,
      this.center = const CustomLatLong(),
  ...
  @override
  State<CustomOpenStreetMapSearchAndPick> createState() =>
      _CustomOpenStreetMapSearchAndPickState();
}

class _CustomOpenStreetMapSearchAndPickState
    extends State<CustomOpenStreetMapSearchAndPick> {
  ...
  Future<CustomPickedData> pickData() async {
    CustomLatLong center = CustomLatLong(
    ...
    // Placemark detailedAddress = await CurrentLocation.getAddress(
    //     lat: _mapController.center.latitude,
    //     long: _mapController.center.longitude);

    // print(detailedAddress.country);

    return CustomPickedData(center, displayName, decodedResponse["address"]);
  }
}
...
class CustomLatLong {
  ...
  const CustomLatLong({this.latitude = 0.0, this.longitude = 0.0});
}

class CustomPickedData {
  final CustomLatLong latLong;
  final String addressName;
  final Map<String, dynamic> address;
  // final Placemark detailedAddress;

  CustomPickedData(this.latLong, this.addressName, this.address);
}
